    ...
    <div className="h-full flex flex-col">
      <div className="mb-6">
        ...
        <p className="text-gray-400 text-sm mt-1">
          Ideas → Scripts → Thumbnails → Published
        </p>
      </div>
                ...
                {items.length === 0 ? (
                  ...
                  items.map((item) => {
                    const agent = item.agent_id
                      ? getAgentById(item.agent_id)
                      : null;
                    return (
                      <div
                        key={item.id}
                        className="bg-[#1e1e1e] border border-[#333] rounded-lg p-3 hover:border-[#555] transition-colors cursor-pointer group"
                      >
